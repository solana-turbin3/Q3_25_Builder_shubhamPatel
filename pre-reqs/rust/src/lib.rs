...
use solana_program::{pubkey::Pubkey, system_instruction::transfer, hash::hash,system_program, instruction::AccountMeta};
use solana_sdk::{
...
fn send_small_amount() {
    let keypair = read_keypair_file("devwallet.json").expect("Couldn't find wallet file");
    ...
    let to_pubkey = Pubkey::from_str("7nsSD1wnDvmq7LtF2ZXK8aiNowzMbnEkxDqjCHqYyuvh").unwrap();
...
#[test]
fn prereq_rs() {
    let signer = read_keypair_file("../ts/turbine_wallet.json").expect("Couldn't find wallet file");
    ...
    let seeds2 = &[b"collection", collection.as_ref()];
    let (prereq_pda2, _bump2) = Pubkey::find_program_address(seeds2, &turbin3_prereq_program);
    ...
    let accounts = vec![
        ...
        AccountMeta::new_readonly(prereq_pda2, false), // authority (PDA)
